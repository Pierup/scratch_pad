...
- (IBAction)buy:(id)sender{
    ...
    NSString* server_url = @"http://fake-merchant.elasticbeanstalk.com/server/sdk/pay/AAA000000001";
    NSString* shop_name = @"Pier Furniture";
    //NSString* merchant_id = self.merchant_id;
    NSString* merchant_id = @"AAA000000001";
    
    [_merchantParam setValue:merchant_id forKey:@"merchant_id"];
    [_merchantParam setValue:[NSString stringWithFormat:@"%f",self.totalAmount] forKey:@"amount"];
    [_merchantParam setValue:currency forKey:@"currency"];
    [_merchantParam setValue:server_url forKey:@"server_url"];
    [_merchantParam setValue:[self getRandomNumber:1000000000 to:10000000000] forKey:@"order_id"];
    [_merchantParam setValue:shop_name forKey:@"shop_name"];
    [self showSheet];
}
...
- (void)showSheet {
    UIActionSheet *actionSheet = [[UIActionSheet alloc]
                                  initWithTitle:@"Choose your payment option"
                                  delegate:self
                                  cancelButtonTitle:@"Cancel"
                                  destructiveButtonTitle:nil
                                  otherButtonTitles:@"Pay Now (iOS SDK)", @"Pay With Pier App",nil];
